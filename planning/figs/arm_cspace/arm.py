...
def affine_transform(geom, mat):
    ...
    Args:
        ...
        mat - 3x3 homogenous transformation matrix.
...
class Arm(object):
    ...
    def forward_kinematics(self, link, geom):
        ...
        for i in range(link + 1):
            if i == 0:
                trans_mat = trans_matrix(0, 0)
            else:
                trans_mat = trans_matrix(self.lengths[i-1], 0)
            rot_mat = rot_matrix(self.angles[i])
            cur_mat = cur_mat.dot(trans_mat).dot(rot_mat)
        return affine_transform(geom, cur_mat)
